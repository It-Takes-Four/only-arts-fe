		...
		<div className="container mx-auto px-4 py-8">
			<div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-start">
				...
				<div className="overflow-hidden shadow-lg">
					{
						artwork.imageFileId ? (
							<img
								src={collectionService.getArtworkImageUrl(artwork.imageFileId)}
								alt={artwork.title}
				...
				<div className="grid gap-6">
					<h1 className="text-4xl font-extrabold tracking-tight lg:text-5xl">{artwork.title}</h1>
					...
					<p className="text-lg leading-relaxed text-gray-700 dark:text-gray-300">{artwork.description}</p>

					<div className="grid gap-2 text-sm text-muted-foreground">
						<div className="flex items-center gap-2">
							<CalendarDays className="w-4 h-4"/>
							<span>Posted: {new Date(artwork.datePosted).toLocaleDateString()}</span>
						</div>
						{artwork.tokenId && (
							<div className="flex items-center gap-2">
								<Tag className="w-4 h-4"/>
								<span>Token ID: {artwork.tokenId.toString()}</span>
							</div>
						)}
