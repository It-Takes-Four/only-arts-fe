...
export class ArtService extends BaseService {
	...
	async getArtworkById(artworkId: string) {
		try {
			...
			throw new Error(
				error.response?.data?.message || "Failed to get artwork"
			);
		}
	}
	...
	async likeArtwork(artId: string, userId: string): Promise<any> {
		try {
			const response = await this._axios.post("/art-like/like", {
				userId,
				artId,
			});
			return response.data;
			...
			throw new Error(
				error.response?.data?.message || "Failed to like artwork"
			);
		}
	}

	async unlikeArtwork(artId: string, userId: string): Promise<any> {
		try {
			const response = await this._axios.post("/art-like/unlike", {
				userId,
				artId,
			});
			return response.data;
