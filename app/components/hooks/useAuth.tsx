...
export function useAuth() {
  ...
  const isInitialLoading = !isClient || (isClient && hasToken && query.isLoading && !query.data && !query.error);
  ...
  const refreshUser = async () => {
    try {
      ...
      resetLogoutState(); // Reset logout state after successful refresh
      return userData;
    } catch (error) {
  ...
  const performUserRefresh = async () => {
    try {
      ...
      // SECOND: Clear old user data completely
      queryClient.setQueryData(['auth', 'user'], null);
      queryClient.removeQueries({ queryKey: ['auth', 'user'] });

      // THIRD: Reset logout states
      resetLogoutState();
      ...
      // SIXTH: Mark queries as fresh (no need to refetch since we just set fresh data)
      queryClient.invalidateQueries({
        queryKey: ['auth', 'user'],
        refetchType: 'none' // Don't refetch, we just set fresh data
      });
  ...
  return {
    user: isLoggedOut ? null : query.data, // Force null if logged out
    isLoading: isLoggedOut ? false : isInitialLoading, // Use the more accurate loading state
    isAuthenticated: isLoggedOut ? false : (hasToken && !!query.data), // Force false if logged out, true if we have token and user
    error: query.error,
