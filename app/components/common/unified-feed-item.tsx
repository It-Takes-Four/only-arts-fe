  ...
  const getDisplayData = () => {
    switch (item.type) {
      case 'post':
        ...
        const postTitle = item.post?.title || `${item.post?.artistName || 'Unknown Artist'} shared a post`;
      ...
      case 'collection':
        return {
          title: item.collection?.collectionTitle || 'Untitled Collection',
          content: item.collection?.collectionDescription || '',
          artistName: item.collection?.artistName || '',
          artistId: item.collection?.artistId || '',
          artistProfileFileId: item.collection?.artistProfileFileId || null,
  ...
  return (
    <motion.article
      className={cn(
        ...
        displayData.type === 'art' && "border-purple-200/50 hover:border-purple-300/50",
        displayData.type === 'collection' && "border-blue-200/50 hover:border-blue-300/50", 
        displayData.type === 'post' && "border-green-200/50 hover:border-green-300/50"
      )}
    ...
    >
      ...
      <div className={cn(
        "p-4 pb-3",
        displayData.type === 'art' && "bg-gradient-to-r from-purple-50/30 to-pink-50/30 dark:from-purple-900/10 dark:to-pink-900/10",
        displayData.type === 'collection' && "bg-gradient-to-r from-blue-50/30 to-cyan-50/30 dark:from-blue-900/10 dark:to-cyan-900/10",
        displayData.type === 'post' && "bg-gradient-to-r from-green-50/30 to-emerald-50/30 dark:from-green-900/10 dark:to-emerald-900/10"
      )}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="flex items-center space-x-1 text-xs text-muted-foreground">
                ...
                <span>{formatDate(displayData.createdDate)}</span>
              </div>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            displayData.type === 'art' && "from-purple-400 to-pink-400",
            displayData.type === 'collection' && "from-blue-400 to-cyan-400",
            displayData.type === 'post' && "from-green-400 to-emerald-400"
          )}>
        ...
        {displayData.title && (
          <h2 className={cn(
            ...
            displayData.type === 'post' ? "text-base line-clamp-4" : "text-lg line-clamp-2"
          )}>
            {displayData.title}
          </h2>
        )}
        ...
        {displayData.type === 'art' && displayData.tags && displayData.tags.length > 0 && (
      ...
      {hasImage && displayData.imageFileId && (
        <div className={cn(
            ...
            <AuthenticatedImage
              ...
              errorComponent={
                <div className="w-full h-full bg-muted flex flex-col items-center justify-center p-4">
                  <div className="text-xs text-muted-foreground mb-2">Failed to load image</div>
          ...
          <div className={cn(
            ...
            displayData.type === 'art' && "from-purple-900/30 to-transparent",
            displayData.type === 'collection' && "from-blue-900/30 to-transparent"
          )} />
        </div>
      )}
      ...
      <div className="p-4 pt-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-red-500 transition-colors">
              <Heart className="w-4 h-4 mr-1" />
              <span className="text-xs">0</span>
            </Button>
            
            <Button variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-blue-500 transition-colors">
              <MessageCircle className="w-4 h-4 mr-1" />
              <span className="text-xs">0</span>
            </Button>
            
            <Button variant="ghost" size="sm" className="h-8 px-2 text-muted-foreground hover:text-green-500 transition-colors">
