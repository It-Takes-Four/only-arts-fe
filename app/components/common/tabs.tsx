...
>(({ className, ...props }, ref) => (
	<TabsPrimitive.List
		ref={ref}
		className={cn(
			"inline-flex items-center justify-start overflow-hidden bg-secondary/50 border border-border/25 rounded-2xl p-2",
			className,
		)}
		{...props}
	/>
		...
		<div className="relative">
			{isActive && (
				<motion.div
					...
					className="absolute inset-0 rounded-lg bg-foreground shadow-[rgba(0,0,0,0.04)_0px_1px_6px] dark:shadow-[rgba(0,0,0,0.2)_0px_1px_6px]"
				/>
			)}
...
>(({ className, ...props }, ref) => (
	<TabsPrimitive.Content
		ref={ref}
		className={cn(
			"focus-visible:ring-ring relative mt-2 rounded-xl ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
			className,
		)}
		{...props}
	/>
