	...
	return (
		<motion.div
			...
			className={`group relative overflow-hidden ${
				isSingleColumn ? "rounded-2xl" : "rounded-xl"
			} cursor-pointer bg-background/80 backdrop-blur-sm border border-border/50 shadow-lg`}
					...
					<div className="min-h-[200px] bg-gradient-to-br from-primary/10 via-secondary/5 to-primary/10 flex items-center justify-center p-6">
						<div className="text-center space-y-3">
							<motion.h3
								...
								style={{
									background:
										"linear-gradient(90deg, currentColor 0%, transparent 50%, currentColor 100%)",
					...
					>
						...
						>
							<Avatar className="h-8 w-8 border-2 border-white/20">
								<AvatarImage
									src={getUserProfileImageUrl() || undefined}
									alt={post.artist.user.username}
								/>
						...
						>
							<h3
								className={`${
									isSingleColumn ? "text-xl" : "text-lg"
							...
							{post.imageUrl && (
								<p
									className={`${
										isSingleColumn ? "text-base" : "text-sm"
									} text-white/90 mb-3 line-clamp-2`}
								>
									{post.content}
								</p>
							)}
